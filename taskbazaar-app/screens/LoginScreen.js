...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    try {
      const res = await axios.post('http://192.168.10.9:5000/api/auth/login', { email, password });
      ...
      // *** This is the conditional navigation logic ***
      if (user.role === 'provider') {
        navigation.navigate('ProviderDashboard');
      } else {
        // This will navigate to Aniqa's dashboard for regular users
